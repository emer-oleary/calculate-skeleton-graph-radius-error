...
def rotation_matrix_from_axis_angle(axis, angle):
    """Helper function to create rotation matrix from axis and angle"""
...
def _print_results_table(
    ...
    r_eff_min: float = 0.5,
        ...
        Sample | Radius Estimate (t_cl) | Measured Radius (r_eff, clamped)
               | CDM Value (cdm_val) | Signed Relative Error (%) | Absolute Relative Error (%)
    
        Ground truth = r_eff (clamped to >= r_eff_min)
        Estimate = t_cl
        """
    
        t_cl = np.asarray(t_cl, dtype=float)
        r_eff_raw = np.asarray(r_eff, dtype=float)
        cdm_val = np.asarray(cdm_val, dtype=float)
    
        n = min(len(t_cl), len(r_eff_raw), len(cdm_val))
        t_cl = t_cl[:n]
        r_eff_raw = r_eff_raw[:n]
        cdm_val = cdm_val[:n]
    
        # --- Enforce minimum effective radius ---
        r_eff_clamped = np.maximum(r_eff_raw, r_eff_min)
    
        # --- Relative error calculations (r_eff is ground truth) ---
        signed_rel_err = (t_cl - r_eff_clamped) / r_eff_clamped * 100.0
        abs_rel_err = np.abs(signed_rel_err)
    
        header = (
            f"{'Sample':>6}  "
            f"{'Radius Estimate (t_cl)':>22}  "
            f"{'Measured Radius (r_eff)':>24}  "
            f"{'CDM Value (cdm_val)':>18}  "
            f"{'Signed Relative Error':>21}  "
            f"{'Absolute Relative Error':>23}"
        )
        print(header)
        print("-" * len(header))
        ...
        for i in range(n):
            print(
                f"{i+1:6d}  "
                f"{t_cl[i]:22.6f}  "
                f"{r_eff_clamped[i]:24.6f}  "
                f"{cdm_val[i]:18.6f}  "
                f"{signed_rel_err[i]:20.3f}%  "
                f"{abs_rel_err[i]:22.3f}%"
            )
...
def main():
    ...
    image_read_path = r"D:\Skeletonization\Methods & Benchmark Paper\Images\Tree_Structure_Lnet_i6_1.tif"
    am_folder_read_path = r"D:\Skeletonization\Methods & Benchmark Paper\Baseline Algorithms\Best Spatial Graphs\Tree\Spatial Graphs"
    
    # ------------------------------------------- User Input: Parameters -----------------------------------------------------
    voxel_size = 1  # voxel size in microns
    sample_n = 100  # number of randomized radius sample points
    subvolume_edge_size_cdm_factor = 6  # 2D Reslice edge size as multiple of chamfer distance map (CDM) value
    
    # ------------------------------------------------------------------------------------------------------------------------

    # Read files 
    tif_path = Path(image_read_path)
    am_folder = Path(am_folder_read_path)

    print(f"\n=== Processing segmentation TIF: {tif_path.name} ===")

    print("Reading segmentation .tif file...")
    tif_volume = tiff.imread(str(tif_path)).astype(bool)

    print("Calculating chamfer distance map...")
    tif_volume_cdm = distance_transform_cdt(tif_volume)

    # Loop through .am files
    am_files = sorted(am_folder.glob("*.am"))
    if not am_files:
        print(f"\nNo .am files found in folder:\n  {am_folder}")
        return

    for am_read_path in am_files:
        print(f"\n--- Processing spatial graph AM: {am_read_path.name} ---")

        # Read spatial graph
        num_vertices, num_edges, num_points, nodes, segments, points_per_segment, points, thickness = read_am(
            str(am_read_path)
        )

        # Measure effective radii
        res = measure_effective_radii(
            tif_volume,                  # binary segmentation
            tif_volume_cdm,              # CDM, same shape
            points,                      # [pid, x, y, z]
        ...
        t_cl = res["t_cl"]                 # spatial/skeleton graph thickness-based radius estimate
        r_eff = res["r_eff"]               # reslice-derived effective radius
        cdm_val = res["max_cdm_values"]    # per-sample CDM value

        _print_results_table(t_cl=t_cl, r_eff=r_eff, cdm_val=cdm_val)
